    ...
    for user in users:
        ...
        if not mfa_devices:
            findings.append({
                ...
                "remediation": cis_rules["iam_user_without_mfa"]["remediation"]
            })
        ...
        for key in access_keys:
            if key['Status'] == 'Active':
                findings.append({
                    ...
                    "remediation": cis_rules["iam_root_access_key"]["remediation"]
                })
            ...
            for stmt in statements:
                if stmt.get('Effect') == 'Allow' and stmt.get('Action') == '*' and stmt.get('Resource') == '*':
                    findings.append({
                        "resource": user_name,
                        "type": "IAM User",
                        "risk": cis_rules["iam_policy_overly_permissive"]["risk_level"],
                        "issue": "Inline policy is overly permissive",
                        "cis_rule": cis_rules["iam_policy_overly_permissive"]["cis_rule"],
                        "remediation": cis_rules["iam_policy_overly_permissive"]["remediation"]
                    })
            ...
            for stmt in statements:
                if stmt.get('Effect') == 'Allow' and stmt.get('Action') == '*' and stmt.get('Resource') == '*':
                    findings.append({
                        ...
                        "type": "IAM User",
                        "risk": cis_rules["iam_policy_overly_permissive"]["risk_level"],
                        "issue": f"Attached policy {attached_policy['PolicyName']} is overly permissive",
                        "cis_rule": cis_rules["iam_policy_overly_permissive"]["cis_rule"],
                        "remediation": cis_rules["iam_policy_overly_permissive"]["remediation"]
                    })
    ...
    try:
        ...
        if not password_policy.get('RequireUppercaseCharacters'):
            findings.append({
                ...
                "remediation": cis_rules["iam_password_policy"]["remediation"]
            })
